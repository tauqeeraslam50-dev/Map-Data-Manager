...
function startTileServer() {
  ...
  server = https.createServer(tls, (req, res) => {
    ...
    if (parsed.pathname === '/health') {
      ...
      return res.end(JSON.stringify({ ok: true, root: offlineRoot, satelliteRoot, terrainRoot, port: serverPort, protocol: 'https' }));
    }
...
ipcMain.handle('offline:select-folder', async () => {
  const result = await selectFolder('Select offline base-map tile folder');
  if (result.canceled || !result.filePaths[0]) return null;
  offlineRoot = result.filePaths[0];
  return { path: offlineRoot, tileUrl: `https://127.0.0.1:${serverPort}/tiles/{z}/{x}/{y}.png` };
});
ipcMain.handle('satellite:select-folder', async () => {
  const result = await selectFolder('Select offline satellite tile folder');
  if (result.canceled || !result.filePaths[0]) return null;
  satelliteRoot = result.filePaths[0];
  return { path: satelliteRoot, tileUrl: `https://127.0.0.1:${serverPort}/satellite/{z}/{x}/{y}.png` };
});
ipcMain.handle('terrain:select-folder', async () => {
  const result = await selectFolder('Select offline terrain tile folder');
  if (result.canceled || !result.filePaths[0]) return null;
  terrainRoot = result.filePaths[0];
  return { path: terrainRoot, tileUrl: `https://127.0.0.1:${serverPort}/terrain/{z}/{x}/{y}.png` };
});
  ...
  const walk = dir => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(full);
        ...
        bounds = bounds ? {
          minLat: Math.min(bounds.minLat, b.minLat), maxLat: Math.max(bounds.maxLat, b.maxLat),
          minLng: Math.min(bounds.minLng, b.minLng), maxLng: Math.max(bounds.maxLng, b.maxLng)
        } : b;
      }
    }
  };
  walk(root);
  return { files, tiles, zooms: [...zooms].sort((a, b) => a - b), root, bounds };
}
...
ipcMain.handle('download:tiles', async (_event, options) => {
  ...
  const z0 = Math.max(0, Math.min(19, Number(minZoom)));
  const z1 = Math.max(z0, Math.min(19, Number(maxZoom)));
  const minLat = Math.max(-85.05112878, Math.min(85.05112878, Number(bounds.minLat)));
  const maxLat = Math.max(-85.05112878, Math.min(85.05112878, Number(bounds.maxLat)));
  const minLng = Math.max(-180, Math.min(180, Number(bounds.minLng)));
  const maxLng = Math.max(-180, Math.min(180, Number(bounds.maxLng)));
  let total = 0;
  for (let z = z0; z <= z1; z++) {
    const x0 = lonToX(Math.min(minLng, maxLng), z);
    const x1 = lonToX(Math.max(minLng, maxLng), z);
    const y0 = latToY(Math.max(minLat, maxLat), z);
    const y1 = latToY(Math.min(minLat, maxLat), z);
    total += Math.max(0, x1 - x0 + 1) * Math.max(0, y1 - y0 + 1);
  }
  ...
  for (let z = z0; z <= z1; z++) {
    const x0 = lonToX(Math.min(minLng, maxLng), z), x1 = lonToX(Math.max(minLng, maxLng), z);
    const y0 = latToY(Math.max(minLat, maxLat), z), y1 = latToY(Math.min(minLat, maxLat), z);
    for (let x = x0; x <= x1; x++) {
      for (let y = y0; y <= y1; y++) {
        const tileUrl = template.replace('{z}', z).replace('{x}', x).replace('{y}', y);
        const ext = /\.jpg|\.jpeg/i.test(tileUrl) ? 'jpg' : 'png';
        const file = path.join(destination, String(z), String(x), `${y}.${ext}`);
        if (await downloadTile(tileUrl, file)) downloaded++;
        done++;
        if (mainWindow && done % 10 === 0) mainWindow.webContents.send('download:progress', { done, total, downloaded });
      }
    }
  }
