  ...
  const walk = dir => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(full);
        ...
        if (m) {
          const z=Number(m[1]), x=Number(m[2]), y=Number(m[3]); tiles++; zooms.add(z);
          const b=tileBounds(z,x,y);
          bounds = bounds ? { minLat:Math.min(bounds.minLat,b.minLat), maxLat:Math.max(bounds.maxLat,b.maxLat), minLng:Math.min(bounds.minLng,b.minLng), maxLng:Math.max(bounds.maxLng,b.maxLng) } : b;
        }
      }
    }
  };
...
app.whenReady().then(() => {
  session.defaultSession.setCertificateVerifyProc((request, callback) => {
    if (request.hostname === '127.0.0.1' || request.hostname === 'localhost') callback(0); else callback(-3);
  });
  startTileServer();
  createWindow();
});
