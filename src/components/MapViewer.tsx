...
export default function MapViewer() {
  ...
  const updatePmtilesLayer = useCallback(() => {
    ...
    import('../lib/mapState').then(module => {
      ...
      const boundsToFit: maplibregl.LngLatBoundsLike | null = null;
      ...
      activePackages.forEach(pkg => {
        ...
        try {
          if (pkg.tileType === 1) {
            if (!pkg.vectorLayers || pkg.vectorLayers.length === 0) {
              console.warn(`PMTiles ${pkg.name} is MVT but has no vector source layers.`);
              return;
            }
            ...
            pkg.vectorLayers.forEach((sourceLayer, index) => {
              ...
              map.current?.addLayer({
                ...
                paint: { 'line-color': '#2563eb', 'line-width': 1.2, 'line-opacity': 0.85 }
              }, 'los-line-layer');
            ...
            // Raster PMTiles (PNG/JPEG/WebP/AVIF) is rendered directly through
            // the registered pmtiles:// protocol.
            map.current!.addSource(sourceId, {
              type: 'raster',
              url: archiveUrl,
              tileSize: 256
          ...
          setMapError(`Could not render ${pkg.name}. Check that it is a valid PMTiles archive.`);
        }
      });

      setHasMapLayer(renderedPackageCount > 0);
      if (renderedPackageCount > 0) setMapError(null);

      // Automatically move to the actual archive extent. This fixes the case
      // where the uploaded PMTiles covers Pakistan but the initial map view is elsewhere.
      if (combinedBounds && !combinedBounds.isEmpty()) {
        requestAnimationFrame(() => {
          map.current?.fitBounds(combinedBounds!, { padding: 40, duration: 500, maxZoom: 12 });
        });
      }
    }).catch(error => {
      console.error('Failed to update PMTiles layers:', error);
      setHasMapLayer(false);
      setMapError('PMTiles renderer failed to initialize. Open the browser console for details.');
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const handleMapError = (event: any) => {
      const message = event?.error?.message || event?.error || 'Unknown MapLibre/PMTiles error';
  ...
  const handleTowerClick = (tower: Tower) => {
    setSelectedTowers(prev => {
      ...
      if (prev.length >= 2) return [prev[1], tower];
      return [...prev, tower];
    });
  };
  ...
  useEffect(() => {
    if (selectedTowers.length === 2) {
      ...
      if (map.current && mapLoaded && map.current.getSource(lineSourceId)) {
        ...
        source.setData({ type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: [[t1.lng, t1.lat], [t2.lng, t2.lat]] } });
        map.current.setPaintProperty('los-line-layer', 'line-color', los.isClearLoS ? '#22c55e' : '#ef4444');
      }
    } else {
      setLosResult(null);
      if (map.current && mapLoaded && map.current.getSource(lineSourceId)) {
    ...
    <div className="w-full h-full bg-slate-300 rounded-xl overflow-hidden shadow-inner flex flex-col relative border-4 border-white">
      <div className="absolute top-4 left-4 z-[1000] bg-white/90 backdrop-blur shadow-md rounded-md px-4 py-2 border border-slate-200 flex justify-between items-center space-x-4">
        ...
        <div className="text-xs font-medium text-slate-700">
          ...
          {selectedTowers.length === 2 && <button onClick={() => setSelectedTowers([])} className="text-blue-600 hover:text-blue-800 hover:underline font-bold">Clear Selection</button>}
        </div>
      </div>
      ...
      <div className="relative flex-1">
        ...
        {mapError && (
          <div className="absolute top-20 left-4 right-4 z-[1001] bg-red-50/95 backdrop-blur rounded-md border border-red-200 shadow px-3 py-2 text-xs text-red-700">
        ...
        {losResult && selectedTowers.length === 2 && (
          <div className="absolute bottom-6 left-6 z-[1000] bg-white/95 backdrop-blur rounded-lg border border-slate-300 shadow-2xl p-4 w-[360px]">
            <h3 className="text-xs font-bold text-slate-500 uppercase mb-3 border-b border-slate-200 pb-2">Link Profile: {selectedTowers[0].name} ↔ {selectedTowers[1].name}</h3>
            <div className="flex justify-between items-center mb-4"><div className="bg-blue-50 border border-blue-100 p-2 rounded w-full"><p className="text-[10px] text-blue-600 font-bold uppercase">Air Distance</p><p className="text-lg font-mono font-bold text-blue-900">{losResult.distance.toFixed(2)} km</p></div></div>
            <div className="space-y-2 text-xs font-mono text-slate-600">
              <div className="flex justify-between items-end border-b border-slate-100 pb-1"><span>Max Radio Horizon</span><span className="font-bold text-slate-900">{losResult.maxDistance.toFixed(2)} km</span></div>
              <div className="flex justify-between items-end border-b border-slate-100 pb-1"><span>Earth Bulge (Mid)</span><span className="font-bold text-slate-900">{losResult.earthBulgeMeters.toFixed(2)} m</span></div>
              <div className="flex justify-between items-end border-b border-slate-100 pb-1"><span>60% Fresnel Radius</span><span className="font-bold text-slate-900">{losResult.requiredClearance.toFixed(2)} m</span></div>
              <div className="flex justify-between items-center mt-3 pt-2"><span className="font-bold font-sans text-slate-700">LoS Status:</span><span className={`px-2 py-1 rounded font-bold text-[10px] ${losResult.isClearLoS ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{losResult.isClearLoS ? "CLEAR (60% FRESNEL)" : "OBSTRUCTED"}</span></div>
              {!losResult.isClearLoS && <div className="text-[10px] text-red-500 mt-2 leading-tight font-sans">Earth curvature blocks the required Fresnel zone clearance for this distance and tower height combination.</div>}
            </div>
          </div>
        )}
      </div>
